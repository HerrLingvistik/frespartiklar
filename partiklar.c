...
FBOstruct *initFBOTex(){
	...
	for(i = 0; i < W; i++){
		for(j = 0; j < H; j++){
			...
			texStart[i][j][0] = (float)floor((rand()/(float)RAND_MAX*255.0f));
			texStart[i][j][1] = (float)floor((rand()/(float)RAND_MAX*255.0f));
...
void Display()
{
	...
	//uppdatera mha update shaders kör dessa innan draw call
